        ...
        <div class="max-w-6xl mx-auto flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <h1 class="text-xl font-extrabold tracking-tight">Editar Vehículo: {{ $vehiculo->modelo->marca->nombre ?? 'Marca' }} {{ $vehiculo->modelo->nombre ?? 'Modelo' }}</h1>
                </div>
            </div>
            <div class="hidden sm:flex items-center gap-2 bg-blue-50 border border-blue-100 px-3 py-1.5 rounded-lg text-xs text-blue-700 font-medium">
                <i class="fa-solid fa-pen text-blue-500"></i> Editando registro #{{ $vehiculo->id }}
            </div>
        </div>
    </div>
        ...
        <div class="max-w-6xl mx-auto px-6 py-6 space-y-6">
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                    <div>
                        ...
                        <select id="marca_id" name="marca_id" required
                            ...
                            @php $marcaActualId = old('marca_id', $vehiculo->modelo->marca_id ?? ''); @endphp
                            @foreach($marcas as $marca)
                                <option value="{{ $marca->id }}" {{ $marcaActualId == $marca->id ? 'selected' : '' }}>
                                    {{ $marca->nombre }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="modelo_id" id="modelo_id" required
                            ...
                            @if($marcaActualId)
                                @php
                                    $marcaObj = $marcas->firstWhere('id', $marcaActualId);
                                    $modelosDisponibles = $marcaObj ? $marcaObj->modelos : [];
                                @endphp
                                @foreach($modelosDisponibles as $mod)
                                    <option value="{{ $mod->id }}" {{ old('modelo_id', $vehiculo->modelo_id) == $mod->id ? 'selected' : '' }}>
                                        {{ $mod->nombre }}
                                    </option>
                                @endforeach
                            @endif
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                    <div>
                        ...
                        <input type="number" name="ano" value="{{ old('ano', $vehiculo->ano) }}" required
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="number" name="kilometraje" value="{{ old('kilometraje', $vehiculo->kilometraje) }}" required min="0"
                        ...
                        <div class="flex p-1 bg-gray-100 rounded-xl gap-1">
                            <label class="flex-1 cursor-pointer">
                                <input type="radio" name="transmision" value="Manual" 
                                       {{ old('transmision', $vehiculo->transmision) == 'Manual' ? 'checked' : '' }}
                                       class="peer hidden">
                            ...
                            <label class="flex-1 cursor-pointer">
                                <input type="radio" name="transmision" value="Automática"
                                       {{ old('transmision', $vehiculo->transmision) == 'Automática' ? 'checked' : '' }}
                                       class="peer hidden">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                    ...
                    <div>
                        ...
                        <input type="text" name="motor" value="{{ old('motor', $motor ?? '') }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="precio_venta" value="{{ old('precio_venta', intval($vehiculo->precio_venta)) }}" required
                    ...
                    <div>
                        ...
                        <select name="condicion"
                                class="w-full px-3 py-2.5 border border-gray-200 rounded-xl bg-gray-50 text-sm font-medium text-gray-700 focus:outline-none focus:ring-4 focus:ring-orange-500/10 focus:border-[#eb5e10] focus:bg-white transition">
                            <option value="Usado" {{ old('condicion', $vehiculo->condicion) == 'Usado' ? 'selected' : '' }}>Usado</option>
                            <option value="Nuevo" {{ old('condicion', $vehiculo->condicion) == 'Nuevo' ? 'selected' : '' }}>Nuevo</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-4">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2.5" id="equipamiento-grid">
                    ...
                    $oldEquipamiento = old('equipamiento', $equipamientoArray ?? []);
                    $extrasCustom = array_diff($oldEquipamiento, $extras);
                    @endphp
                ...
                <div id="extras-custom-list" class="mt-3 flex flex-wrap gap-2">
                    @foreach($extrasCustom as $index => $custom)
                        @php $idCustom = 'extra_old_' . $index; @endphp
                        <div class="flex items-center gap-2 bg-orange-50 border border-orange-200 rounded-xl px-3 py-1.5 shadow-sm text-[#eb5e10] font-medium" id="{{ $idCustom }}">
                            <input type="checkbox" name="equipamiento[]" value="{{ $custom }}" checked class="rounded text-[#eb5e10] focus:ring-[#eb5e10] w-3.5 h-3.5 hidden">
                            <span class="text-sm">{{ $custom }}</span>
                            <button type="button" onclick="document.getElementById('{{ $idCustom }}').remove()" 
    ...
    document.getElementById('bloque-solo-contado').classList.toggle('hidden', this.checked);
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 flex flex-col justify-between">
                    <div>
                        ...
                        <div class="grid grid-cols-3 gap-2">
                            {{-- SIN EMOJIS: reemplazados por iconos Font Awesome --}}
                            @foreach([
                            ...
                            <label class="cursor-pointer">
                                <input type="radio" name="estado_publicacion" value="{{ $value }}"
                                       {{ old('estado_publicacion', $vehiculo->estado_publicacion) == $value ? 'checked' : '' }}
                                       class="peer hidden">
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            <label class="cursor-pointer">
                                <input type="radio" name="consignado" value="0"
                                       {{ old('consignado', $vehiculo->consignado) == '0' ? 'checked' : '' }}
                                       class="peer hidden">
                            ...
                            <label class="cursor-pointer">
                                <input type="radio" name="consignado" value="1"
                                       {{ old('consignado', $vehiculo->consignado) == '1' ? 'checked' : '' }}
                                       class="peer hidden">
                                <span class="block text-center text-sm font-bold py-3 border border-gray-100 rounded-xl bg-gray-50/50 text-gray-600 peer-checked:border-[#1a2b4c] peer-checked:bg-[#1a2b4c]/5 peer-checked:text-[#1a2b4c] transition duration-200">
                                    Consignación
                                </span>
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                    <div class="border-b border-gray-100 pb-3 mb-4 flex justify-between items-center">
                        <h2 class="text-sm font-bold uppercase tracking-wider text-gray-400">Nuevas Imágenes</h2>
                        <span class="text-[10px] text-gray-400 font-medium">Se convertirán a WebP automáticamente</span>
                    </div>
                    ...
                    <div id="drop-zone" 
                        ...
                        <div class="flex flex-col items-center gap-2">
                            ...
                            <div>
                                <p class="text-sm font-bold text-gray-700">Arrastra fotos aquí o haz clic para subir</p>
                                <p class="text-xs text-gray-400 mt-0.5">Soporta JPG, PNG, WEBP — Máx 5MB — Se guardan como WebP</p>
                            </div>
                        </div>
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between bg-white rounded-2xl border border-gray-100 p-5 gap-4 shadow-sm">
                <div class="flex items-center gap-2.5 text-xs text-gray-400 font-medium">
                    ...
                    Revisa todos los datos antes de aplicar los cambios en el sistema.
                </div>
...
window.addEventListener('DOMContentLoaded', () => {
    const precioInput = document.querySelector('[name="precio_venta"]');
    if(precioInput.value) precioInput.dispatchEvent(new Event('input'));
});
...
dropZone.addEventListener('dragover', (e) => { e.preventDefault(); dropZone.classList.add('border-[#eb5e10]', 'bg-orange-50/20'); });
dropZone.addEventListener('dragleave', () => { dropZone.classList.remove('border-[#eb5e10]', 'bg-orange-50/20'); });
...
document.getElementById('marca_id').addEventListener('change', function() {
    ...
    if (!marcaId) { modeloSelect.innerHTML = '<option value="">Seleccionar marca primero</option>'; return; }
    modeloSelect.innerHTML = '<option value="">Cargando modelos...</option>';
    fetch(`/api/modelos?marca_id=${marcaId}`)
        .then(res => res.json())
        .then(data => {
            modeloSelect.innerHTML = '<option value="">Seleccionar modelo</option>';
            data.forEach(modelo => { modeloSelect.innerHTML += `<option value="${modelo.id}">${modelo.nombre}</option>`; });
        })
        .catch(() => { modeloSelect.innerHTML = '<option value="">Error al cargar los modelos</option>'; });
});
...
function agregarExtra() {
    ...
    lista.innerHTML += `
        <div class="flex items-center gap-2 bg-orange-50 border border-orange-200 rounded-xl px-3 py-1.5 shadow-sm text-[#eb5e10] font-medium" id="${id}">
            <input type="checkbox" name="equipamiento[]" value="${val}" checked class="rounded text-[#eb5e10] focus:ring-[#eb5e10] w-3.5 h-3.5 hidden">
            <span class="text-sm">${val}</span>
            <button type="button" onclick="document.getElementById('${id}').remove()" class="ml-1 text-orange-400 hover:text-red-500 transition font-bold text-xs">✕</button>
        </div>
    `;
    input.value = '';
    input.focus();
